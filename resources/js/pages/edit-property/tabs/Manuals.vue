        ...
        <div class="wrapper">
            <ul class="navigation" v-if="categories.length > 0">
                ...
                >
                    ...
                    <span class="navigation__count">({{ category.manuals.length }})</span>
            ...
            <div class="container">
                <div class="form">
                    <div v-if="activeCategory">
                        <div class="rules__tab" v-for="(manual, index) in activeCategory.manuals">
                            ...
                            <button class="rules__tab-delete" @click="activeCategory.manuals.splice(index, 1)"></button>
                        </div>

                        <div class="rules__item">
                            <p class="rules__item-name">
                                <input type="text" v-model="newRule.title" placeholder="Enter title of a manual">
                            </p>
                            <p class="rules__item-text">
                                <textarea type="text" v-model="newRule.text" placeholder="Enter manual"></textarea>
                            </p>
                        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            categories: {
                get: function() {
                    return this.valueLocal.manuals_categories ? this.valueLocal.manuals_categories : [];
                },
                set: function(value) {
                    this.valueLocal.manuals_categories = value;
                }
            },
            activeCategory() {
                return this.activeCategoryId !== null ?
                    this.valueLocal.manuals_categories.find(category => category.id === this.activeCategoryId) :
                    null;
            }
        },
        ...
        methods: {
            addRule: function () {
                this.activeCategory.manuals.push(this.newRule);
            ...
            selectActiveCategoryId() {
                if (this.valueLocal &&
                    this.valueLocal.manuals_categories &&
                    this.valueLocal.manuals_categories.length &&
                    !this.valueLocal.manuals_categories.find(item => item.id === this.activeCategoryId)) {
                    this.activeCategoryId = this.valueLocal.manuals_categories[0].id;
                }
            }
        }
    }
